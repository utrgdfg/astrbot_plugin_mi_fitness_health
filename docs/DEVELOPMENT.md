...
- 自然对话的并发刷新会合并为单个后台任务。
- 判断模型默认使用 8 秒硬超时，配置项 `context_decision_timeout_seconds` 可在 3～30 秒之间调整；对话云端刷新默认等待 5 秒，`natural_query_cloud_wait_seconds` 可在 1～30 秒之间调整。可选补充回复草稿模型仍使用 2 秒硬超时；底层 Provider 即使忽略取消也不会继续占用 `on_llm_request` 调用路径。选择了判断模型时，其有效结果是唯一的自然对话数据路由依据；模型超时、失败、输出无效或处于退避期间均 fail closed，本轮不注入健康数据。只有未选择判断模型时才使用本地轻量规则。刷新超时通过 `asyncio.shield` 保留后台任务，用户配置的两项等待上限会顺序影响当前回复延迟。
...
`context_decision_provider_id` 是可选的对话分类模型。它接收当前所有者私聊中经过长度限制和边界转义的最近 `user`/可选 `assistant` 纯文本，以及当前消息，并返回严格 JSON；不接收 system、tool、多媒体内容或本轮刚读取的小米生活数据。历史条数由 `context_decision_message_count` 控制（0～20，总量最多 4000 字符），是否包含 Bot 回复由 `context_decision_include_bot_messages` 控制。有效结果最多选择两个数据类别，随后仍由插件根据每类缓存的新鲜度决定是否联网刷新。选择该模型后，模型失败、超时、输出无效或处于 1、5、15 分钟退避期间均静默跳过本轮健康上下文，不由本地规则接管；有效结果立即清零退避。没有选择模型时才使用本地关键词规则。原始消息的强制刷新意图单独保留，不依赖模型生成的焦点文本。
