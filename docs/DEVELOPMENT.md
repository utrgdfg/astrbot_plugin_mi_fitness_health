...
- 自然对话的并发刷新会合并为单个后台任务。
- 判断模型使用 3 秒硬超时，可选补充回复草稿模型使用 2 秒硬超时；底层 Provider 即使忽略取消也不会继续占用 `on_llm_request` 调用路径。选择了判断模型时，其有效结果是唯一的自然对话数据路由依据；模型超时、失败、输出无效或处于退避期间均 fail closed，本轮不注入健康数据。只有未选择判断模型时才使用本地轻量规则。普通对话最多等待云端 2 秒，明确健康查询最多等待 5 秒；模型与云端等待预算合计不超过约 10 秒，刷新超时通过 `asyncio.shield` 保留后台任务。
