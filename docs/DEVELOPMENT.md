...
- 自然对话的并发刷新会合并为单个后台任务。
- 判断模型使用 3 秒硬超时；底层 Provider 即使忽略取消也不会继续占用 `on_llm_request` 调用路径。普通对话最多等待云端 2 秒，明确健康查询最多等待 5 秒；超时通过 `asyncio.shield` 保留后台刷新任务。
- 操作锁或后台连接任务繁忙时，自然对话立即使用缓存或跳过数据，不创建排队的云端任务。`/健康连接` 只在命令流水线中发送开始提示，实际检查在后台运行并设有包含锁等待的 120 秒上限。
