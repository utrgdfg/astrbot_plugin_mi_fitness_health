...
- 为单 key、单数据集和整次同步增加记录数量及 5 分钟总时限，限流响应同时支持秒数与 HTTP 日期格式的 `Retry-After`。
...
- SQLite schema 升级到 v6：同步成功/失败状态按小米 `userId` 隔离，主动消息记录按所有者隔离；旧健康记录保留，无法确认账号归属的旧同步状态会被清除后重新刷新。
