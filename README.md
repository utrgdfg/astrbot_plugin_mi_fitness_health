...
- `enable_proactive_sleep_reminder` / `sleep_reminder_time`：开启后，每天在设定时间后向所有者的**私聊**发送一次睡前关怀（默认 23:00）
...
也不必先输入指令：例如直接私聊机器人“我昨天睡得怎么样？”、“我今天走了多少步？”或“刚同步了，看看最新心率”，插件会在缓存超过 `natural_query_sync_minutes`（或你明确要求最新数据）时先刷新小米云端缓存，再将可用的历史记录交给对话模型回答。手机端仍要先将手环/手机数据同步到小米健康云；插件无法读取蓝牙实时数据。
...
睡前关怀不会使用群聊：插件只会在所有者主动发起过一对一私聊后，保存该私聊会话作为投递目标；每天最多一次。关闭 `enable_proactive_sleep_reminder` 可完全停用这类主动消息。
