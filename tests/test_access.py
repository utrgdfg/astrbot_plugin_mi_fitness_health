...
class OwnerAccessTest(unittest.TestCase):
    def test_sid_values_are_normalized(self) -> None:
        self.assertEqual(normalize_identifier(" UID: [2914544254] "), "2914544254")
        self.assertEqual(normalize_identifier("Bot ID：「银河系」"), "银河系")

    def test_platform_instance_is_required_for_private_health_data(self) -> None:
        self.assertFalse(
            owner_identifiers_match("2914544254", "", "2914544254", "银河系")
        )
        reason = owner_access_denial_reason(
            owner_platform_id="2914544254",
            owner_platform_instance_id="",
            sender_id="2914544254",
            platform_id="银河系",
    ...
    def test_private_owner_with_matching_platform_instance_is_allowed(self) -> None:
        self.assertIsNone(
            owner_access_denial_reason(
                owner_platform_id="[2914544254]",
                owner_platform_instance_id="「银河系」",
                sender_id="2914544254",
                platform_id="银河系",
                message_type="FriendMessage",
                is_private=True,
