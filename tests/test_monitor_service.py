...
class MonitorServiceTest(unittest.TestCase):
    def test_recent_private_activity_triggers_once_during_late_window(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            database.touch_private_owner_session("owner", "qq:FriendMessage:123", now.astimezone(UTC) - timedelta(minutes=10))
            service = HealthMonitorService(database, "owner", local_zone, True, "00:30", "06:00", 45, 120)
            ...
            self.assertTrue(asyncio.run(service.proactive_cooling_down(now + timedelta(minutes=30))))
            self.assertIsNone(asyncio.run(service.evaluate_late_activity(now + timedelta(minutes=30))))
